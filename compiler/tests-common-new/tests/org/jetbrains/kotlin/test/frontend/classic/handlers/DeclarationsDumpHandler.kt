...
class DeclarationsDumpHandler(
    ...
    override fun processAfterAllModules() {
        ...
        val niPrefix = if (
            testServices.moduleStructure.modules.any { DiagnosticsDirectives.NI_EXPECTED_FILE in it.directives } &&
            testServices.moduleStructure.modules.any { it.languageVersionSettings.supportsFeature(LanguageFeature.NewInference) }
        ) ".ni" else ""
        val expectedFileName = "${testDataFile.nameWithoutExtension}$niPrefix.txt"
