...
fun <T> test(x: T) {
    ...
    val s3: Pair<Int, String?> = <!TYPE_MISMATCH{NI}, TYPE_MISMATCH{NI}, TYPE_INFERENCE_EXPECTED_TYPE_MISMATCH{OI}!>bar(null, null, ::<!TYPE_INFERENCE_NO_INFORMATION_FOR_PARAMETER{OI}!>foo<!>)<!>
    val s4: Pair<Int?, String> = <!TYPE_MISMATCH{NI}, TYPE_MISMATCH{NI}, TYPE_MISMATCH{NI}, TYPE_INFERENCE_EXPECTED_TYPE_MISMATCH{OI}!>bar(null, null, ::<!IMPLICIT_NOTHING_TYPE_ARGUMENT_IN_RETURN_POSITION{NI}, TYPE_INFERENCE_NO_INFORMATION_FOR_PARAMETER{OI}!>foo<!>)<!>

    val s5: Pair<Int, String> = <!TYPE_MISMATCH{NI}, TYPE_MISMATCH{NI}, TYPE_INFERENCE_EXPECTED_TYPE_MISMATCH{OI}!>bar(1, "", ::foo)<!>
