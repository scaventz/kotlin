...
fun test() {
    val s : Set<Int> = <!TYPE_MISMATCH{NI}, TYPE_INFERENCE_EXPECTED_TYPE_MISMATCH{OI}!>newList()<!>
    use(s)
}
...
fun test1(outA: Out<A>, inB: In<B>) {
    ...
    val b: Two<A, C> = <!TYPE_MISMATCH{NI}, TYPE_INFERENCE_EXPECTED_TYPE_MISMATCH{OI}!>foo(outA, inB)<!>
    use(b)
}
